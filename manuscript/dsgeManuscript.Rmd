...
equivalent to baseline. About `r round(mean(df$y<0)*100,0)`\% of permutations had better predictive performance than the baseline model.
...
the baseline model. -->
A total of `r sum(llike < llike[truePerm])` of permutations resulted in a lower negative
penalized log likelihood. A Bayesian interpretation of these results would be
that `r sum(llike < llike[truePerm])` of the permuted "models" are preferable to the true, unpermuted,
...
df = SummaryStats %>% group_by(nestim) %>% 
  # would be better to have llike from only the 1000 obs after the training
  # period, but this would require rerunning the whole thing since I screwed
  # this up the first time
  do(quant(-.$logPred / (2100 - .$nestim))) 
df %>% ggplot(aes(x=nestim)) +
  geom_ribbon(aes(ymin=`5%`,ymax=`95%`), fill='grey90') + 
  geom_ribbon(aes(ymin=`10%`,ymax=`90%`), fill='grey80') + 
  geom_ribbon(aes(ymin=`20%`,ymax=`80%`), fill='grey70') + 
  ...
  geom_hline(yintercept = -2.135, color = "blue") +
  xlab('number of training points') + 
...
## Best model by predictive log-likelihood
...
I think it's also important to note here, that the penalized negative log-likelihood is very flat relative to permutations. In terms of that metric, `r signif(mean(llikeTest/llikeTest[truePerm] < 1.10),2)*100`% of permutations are within 10% of the true permutation. In the Appendix, we rewrite the beginning of \citet{SmetsWouters2007} permuting all the series to reflect the best one by negative predictive log likelihood.
