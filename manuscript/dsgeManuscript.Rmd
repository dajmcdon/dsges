...
percent_better_models <- round(mean(df$y<0)*100,0)
logperc %>% gather() %>%
...
of about `r percent_better_models`\% which forecast better.
...
It is also important to note here, that the penalized negative log-likelihood 
(in-sample) is very flat relative to permutations. In terms of that metric, 
`r signif(mean(llikeTest/llikeTest[truePerm] < 1.10),2)*100`% of permutations 
are within 10% of the true permutation. In the Appendix, we take this model to 
...
examining the distribution of Taylor parameters conditional on the
truth generating the model and estimating said parameters in the simulation exercise.
\footnote{This analysis is not constrained to 
the distribution of plausible Taylor rule parameters that the Fed might
consider, but rather simply investigates all possible such parameters.} These correlations are shown in \autoref{tab:covary-with-Taylor}. The
...
actually address their original motivation.
...
is the discount factor applied by households. As in \citep{ChristianoEichenbaum2005}, a
