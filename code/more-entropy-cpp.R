...
Rcpp::sourceCpp("code/kalman-like.cpp")
...
for (i in seq_along(ns)) {
  ...
  ii <- seq(ns[i])
  truth[i] <- -1 * ssmodel_ll(long[,ii], ss_true)
  other[i] <- -1 * ssmodel_ll(long[,ii], ss_est)
}

tib <- tibble(n = ns, true_params = truth, est_params = other) %>% 
  pivot_longer(-n, values_to = "neg_loglikelihood")
saveRDS(tib, file = "data/entropy-investigation.RDS")
